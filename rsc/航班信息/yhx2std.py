...
for j in D:
    ...
    # 居然发现有 bug，上面那条不能判断超过 24 小时的车程
    if j['duration'][0]:
        j['end'][0] = j['duration'][0]

# 更换名字
S = re.findall(u'([\u4e00-\u9fa5]+)\|([A-Z]+)',
               open("reponse.text", "r").read())
...
# 去除不存在的车站
DistanceInformation = json.loads(open("DistanceInformation.json", "r").read())

Cities = []
for city in DistanceInformation:
    Cities.append(city)

FinalDataBase = []
for j in D:
    if j["from"] in Cities and j["to"] in Cities:
        FinalDataBase.append(j)

with open("database.json", "w") as f:
    print(FinalDataBase, file=f)
