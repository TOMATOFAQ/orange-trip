...
json money_dijkstra(string from, string to, vector<int> t) {
    // 初始化
    set<string> vis = {from};
    ...
    // dijkstra 过程
    priority_queue<json, vector<json>, cmp_schedule_money_increase> pq;
...
//返回目标节点的status，包括花费金钱，花费时间以及 schedule 序列
json time_dijkstra(string from, string to, vector<int> t) {
    // 初始化
    set<string> vis = {from};
    json least_cost_schedule = {
        {"start", {0, 0, 0}}, {"end", {MAX, MAX, MAX}}, {"price", MAX}};
    ...
    // dijkstra 过程
    priority_queue<json, vector<json>, cmp_schedule_time_increase> pq;
    string current_city = from;
    while (!vis.count(to)) {
        // 更新新加节点后所的各节点状态
        for (json s : G[current_city]) {
...
int main() {
    ...
    json to_status;
    to_status = money_dijkstra("Beijing", "Shenzhen", t);
    for (json p : to_status) {
        cout << p << endl;
    }
    to_status = time_dijkstra("Moscow", "Shenzhen", t);
    for (json p : to_status) {
        cout << p << endl;
    }
