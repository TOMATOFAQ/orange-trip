...
class Algorithm {
   public:
    static json Dijkstra(string from, string to, string priority,
                         vector<int> start_time) {
        if (priority == "time") {
            return Algorithm::dijkstra<struct cmp_schedule_time_increase>(
                from, to, start_time);
        } else {
            return Algorithm::dijkstra<struct cmp_schedule_money_increase>(
                from, to, start_time);
        }
    }

    static void formatted_output(json plan) {
        cout << "Plan Message:" << endl;
        cout << "Arrived time:" << plan["t"] << endl;
        cout << "Total price:" << plan["m"] << endl;
        cout << "Schedules:" << endl;
        for (json p : plan["s"]) {
            cout << p << endl;
        }
   ...
   private:
    // 给当前时间 t1加上一段绝对时间t2
    static vector<int> addtime_absolutely(vector<int> t1, vector<int> t2) {
        t1[2] = t1[2] + t2[2];
        if (t1[2] >= 60) {
            t1[2] = t1[2] - 60;
            t1[1] += 1;
    ...
    static vector<int> addtime_by_schedule_by_start_time(vector<int> t,
                                                         json s) {
        vector<int> ss = s["start"];
        vector<int> se = s["end"];
        if (t[1] > ss[1] || (t[1] == ss[1] && t[2] > ss[2])) {
    ...
    static json chronologize(json sl) {
    ...
    template <typename T>
    static json dijkstra(string from, string to, vector<int> t) {
        // 初始化
        set<string> vis = {from};
...
int main() {
    ...
    to_status = Algorithm::Dijkstra("北京", "广州", "time", start_time);
    Algorithm::formatted_output(to_status);

    to_status = Algorithm::Dijkstra("北京", "广州", "money", start_time);
    Algorithm::formatted_output(to_status);
